...
overdispersion are deliberately excluded (Step 2 adds `grand_overdispersion_rho`; the study RE that
...
- **Single global `phi_md`, not per-study.** Gilman/Levine are single-dose (all 10⁵)
  ...
  overdispersion parameter `grand_overdispersion_rho` / residual. Per-study or ratio-preserving φ is a documented alternative.
...
`grand_overdispersion_rho`, NOT a study/cohort random effect. The RE is withdrawn, not deferred.

    rho = grand_overdispersion_rho ~ beta(1, 49)      # ICC, in [0,1]; rho = 0 IS the binomial
    k   = grand_concentration_k = (1 - rho) / rho     # beta-binomial concentration (derived)
    y_j ~ beta_binomial(n_j, p_j * k, (1 - p_j) * k)  # grouped rows (n_j > 1) only

`p_j` is unchanged (the existing `obs_prob()` value). Sample the **ICC**, not the
concentration: rho rises with dispersion (a name with "overdispersion" on a
*concentration* reads backwards), rho = 0 is an attainable boundary rather than a
limit at k -> Inf, and the variance inflation is the readable design effect
`1 + (n-1)rho`. Do not reuse `kappa` — that is the η dose-scaling parameter.

Anchor: measured excess logit variance across the five Maryland 1e5 cohorts gives
sigma ~ 0.26 -> k ~ 61 -> **rho ~ 0.016**; `Beta(1,49)` brackets it. Design effects
n=16 -> 1.24, n=36 -> 1.57, n=116 -> 2.87 (the largest landing on `H-F-5`, correctly:
that row is a documented pool of many challenges).

Apply to `n_j > 1` rows only; the Darton per-subject rows are n=1 Bernoulli where
overdispersion is not identified. Delete the now-dead `sigma_study` declaration and
...
priorsense on `grand_overdispersion_rho` (expect it prior-leaning, and say so if it is); LOO must
